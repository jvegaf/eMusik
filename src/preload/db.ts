...
import { Playlist, PlaylistModel, Track, TrackModel } from './emusik';
...
const Playlists = new PouchDB<Playlist>(path.join(pathUserData, 'PlaylistsDB'), {
  adapter: 'leveldb',
  auto_compaction: true,
});
...
async function reset(): Promise<void> {
  ...
  // empty it
  const [allTracks, allPlaylists] = await Promise.all([tracks.getAll(), playlists.getAll()]);
  ...
  await Promise.all([Tracks.bulkDocs(deletedTracks), Playlists.bulkDocs(deletedPlaylists)]);
}
...
const tracks = {
  async getAll(): Promise<TrackModel[]> {
    ...
    const tracks = [...firstResponse.rows, ...secondResponse.rows].map(record => record.doc).filter(Boolean);

    return tracks as TrackModel[];
  },
...
const playlists = {
  async getAll(): Promise<PlaylistModel[]> {
    ...
    const playlists = [...firstResponse.rows, ...secondResponse.rows].map(record => record.doc).filter(Boolean);

    return playlists as PlaylistModel[];
  },
